	...
	*/
	...
	for (int epochs = 1; epochs <= 1000; epochs++)
	{
		...
		for (int i = 0; i < 1000; i++)
		{
			...
			int train_res;
			train_res = Net.Train(&TrainDATA[Index], &TrainLabel[Index]);
			if (train_res != target_train[Index])Train_Error++;
		}
		Train_Error_rate = (((double)Train_Error) / 1000.)*100.;

		for (int i = 0; i < 100; i++)
		{
			int Index = rand() % 1000;
			int test_res;
			test_res = Net.Feed_forward(&TestDATA[Index]);
			if (test_res != target_test[Index])Test_Error++;
		}
		Test_Error_rate = (((double)Test_Error) / 100.)*100.;
